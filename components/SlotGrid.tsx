      ...
      <ScrollView style={styles.slotGrid}>
        {rows.map((row, rowIndex) => (
          <View key={`row-${rowIndex}`} style={styles.slotRow}>
            {row.map((slot) => (
              <SlotCard key={slot.id} slot={slot} showLogo={meta.showTeamLogos} />
            ))}
          </View>
        ))}
      </ScrollView>
...
const SlotCard: React.FC<{ slot: SlotEntry; showLogo: boolean }> = ({ slot, showLogo }) => {
  return (
    <Animated.View 
      ...
      >
        <View style={styles.slotInfo}>
          ...
          <View style={styles.teamContainer}>
            ...
            <Text style={[styles.teamName, !showLogo && styles.teamNameFullWidth]} numberOfLines={1}>
              {slot.name}
            </Text>
          </View>
        </View>
